-- The 10,000,000 events: 8M analytics + 2M error, plus 8M transactions and
-- ~500k sessions, spread across 90 daily partitions.
--
...
WITH days AS (
  ...
  FROM generate_series(DATE '2026-05-27', DATE '2026-08-24', INTERVAL '1 day') d
), tot AS (SELECT sum(w) AS sw FROM days)
SELECT day_idx, day,
       floor(8000000 * w / sw)::bigint AS n_analytics,
       floor(2000000 * w / sw)::bigint AS n_errors,
       floor( 500000 * w / sw)::bigint AS n_sessions
FROM days, tot;

UPDATE seed_plan SET
  n_analytics = n_analytics + (8000000 - (SELECT sum(n_analytics) FROM seed_plan)),
  n_errors    = n_errors    + (2000000 - (SELECT sum(n_errors)    FROM seed_plan)),
  n_sessions  = n_sessions  + ( 500000 - (SELECT sum(n_sessions)  FROM seed_plan))
...
BEGIN
  FOR p IN SELECT * FROM seed_plan ORDER BY day_idx LOOP
    ...
    IF p.day_idx % 10 = 0 OR p.day_idx = 1 THEN
      RAISE NOTICE 'day % / 90 (%) done, elapsed %',

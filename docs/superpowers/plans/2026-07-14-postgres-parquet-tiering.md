...
- **Workspace conventions (verbatim):** edition `2021`, rust-version `1.82`, license `LGPL-3.0-only`, `version = "0.1.0"`. Enum-like columns are `TEXT` (not PG enums). Diesel uses `postgres_backend` (no libpq linked into Rust). All DB I/O goes through `&mut AsyncPgConnection`. Config is hand-rolled in `sauron-core::config` via `var()`/`parse()`.

...
/// work to do. Apps are never implicitly ready here: the person/environment rollup has
...
pub async fn backfill_pending(conn: &mut AsyncPgConnection) -> QueryResult<bool> {
    ...
    let r: Present = diesel::sql_query(
        "SELECT EXISTS (SELECT 1 FROM apps a \
                        WHERE NOT EXISTS (SELECT 1 FROM event_user_env_backfill b WHERE b.app_id = a.id)) \
                AS present",
    )
    .get_result(conn)
    .await?;

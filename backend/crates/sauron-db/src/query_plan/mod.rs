...
mod tests {
    ...
    fn every_declared_dimension_lowers_or_is_explicitly_deferred() {
        // 18 since `screen`, `distinctId` and `deviceKey` joined Issues — each
        // an `error_events` column reached through a correlated EXISTS, like
        // `workflow` before them — and then `extra` and `contexts`, the two
        // dev-supplied JSONB columns, over the same bridge.
        // `assert_full_coverage` below is what makes this number mean
        // something: it drives every declared operator on each of them,
        // including the `In` and `Has` that `OPS_EQ`/`OPS_TEXT` grant and that
        // `workflow`'s `OPS_WORKFLOW` does not.
        assert_eq!(dimensions_for(Resource::Issues).count(), 26);

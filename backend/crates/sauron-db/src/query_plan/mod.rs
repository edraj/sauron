...
mod tests {
    ...
    fn every_declared_dimension_lowers_or_is_explicitly_deferred() {
        ...
        assert_eq!(dimensions_for(Resource::Issues).count(), 12);
        assert_eq!(dimensions_for(Resource::Occurrences).count(), 19);
        assert_eq!(dimensions_for(Resource::Events).count(), 8);

        let fixed = Uuid::nil();
        assert_full_coverage(Resource::Issues, &IssuesLower { app_id: fixed });

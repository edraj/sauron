...
-- after, because the planner never chooses `error_events_app_env_time_idx` for
-- this statement at all. A payload-less index scan would have to visit the
...
--
-- 23.6x fewer buffers, 3.8x faster. The index measured 17 MB against a 696 MB
...
-- `error_events_distinct_idx (app_id, distinct_id, occurred_at DESC)`, with
-- Heap Fetches: 0. `One(env)` was the only selection shape without a covering
-- index. This gives it one.
--

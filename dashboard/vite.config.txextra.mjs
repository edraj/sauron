...
/**
 * Dev config for verifying table sorting + pagination (transaction tags/extra).
 *
 ...
 */
const API = process.env.TXEXTRA_API_BASE ?? 'http://localhost:8130';

function configOverride() {
  return {
    name: 'txextra-config-override',
...
export default defineConfig({
  ...
  publicDir: 'static',
  server: { port: 3030, host: true, strictPort: true },
});

...
/**
 * Render harness for the Performance → transactions drill-down.
 *
...
const SERIES = Array.from({ length: 14 }, (_, i) => ({
  bucket: `2026-08-${String(i + 5).padStart(2, '0')}T00:00:00Z`,
  p95: 600 + i * 90,
  throughput: 40 + i * 7,
}));
...
function stubApi() {
  const root = fileURLToPath(new URL('./perfdrill-harness', import.meta.url));
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (path === '/config.js') {
          ...
          res.end(
            `window.__SAURON_CONFIG__ = ${JSON.stringify({
              apiBaseUrl: 'http://localhost:3038',
              ingestBaseUrl: 'http://localhost:3038',
            })};\n`,
          );
          return;
        }
        ...
        if (path === '/v1/apps/app1/transactions') {
          ...
          console.log(`[perfdrill] transactions ${req.url}`);
        ...
        if (path?.startsWith('/v1/')) {
          console.log(`[perfdrill] unstubbed ${req.method} ${path}`);
          return json(res, []);
        }
...
export default defineConfig({
  ...
  root: fileURLToPath(new URL('./perfdrill-harness', import.meta.url)),
  server: { port: 3038, strictPort: true },
});
